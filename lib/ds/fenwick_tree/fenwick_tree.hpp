...
struct FenwickTree {
  ...
  FenwickTree() = default;
  explicit FenwickTree(uint32_t _n) { build(_n); }

  template <typename F>
  FenwickTree(uint32_t _n, F&& func) { build(_n, func); }

  template <typename It>
  FenwickTree(It first, It last) { build(first, last); }
  ...
  void build(uint32_t _n, F&& func) {
    ...
    for (uint32_t i = 1; i <= n; ++i) {
      ...
      t[i] = T::op(pre[i], T::inv(pre[i & (i - 1)]));
    }
  }
